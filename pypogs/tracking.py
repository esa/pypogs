...
- :class:`pypogs.ControlLoopThread` is the main control loop for pypyogs. It manages the transition logic and feedback loop.

- :class:`pypogs.TrackingThread` is the target tracking loop. One is created for each camera to read arriving images. It provides
  the current error etc. to the pypogs.ControlLoopThread.

- :class:`pypogs.SpotTracker` is attached to each pypogs.TrackingThread and implements the spot detection and tracking.
...
class ControlLoopThread:
    ...
    This thread holds a reference to the *parent* pypogs.System instance and takes control of all devices. Set up
    this thread with your desired tracking parameters (such as gains, frequency, feedback maximum rates, etc.) and then
    call ControlLoopThread.start(). The thread runs with a maximum frequency defined by max_frequency,
    it defaults to 5 Hz. The target (and start and end times if desired) must be defined in the *parent*.target object.

    Upon reaching a transition criteria, the tracking mode will be changed (e.g. open-loop to closed-loop)
    automatically either to a "better" or "worse" mode. The available modes and their parameters are defined below:

        - OL: Open-loop control. The mount angles are read out and the control signal is based on the difference
          between the mount angles and the target angles. Gains OL_P (proportional, default 1.0), OL_I (integral,
          default 10 seconds), and rate limit OL_speed_limit (default 3600 arcsec/s) apply.

        - CCL: Coarse closed-loop control. The measured position from the coarse camera, returned by
          *parent*.coarse_track_thread.track_x_y, is used (scaled to degrees) for the control signal.
          Gains CCL_P (proportional, default .5), CCL_I (integral, default 10 s), and rate limit CCL_speed_limit
          (default 180 arcsec/s) apply. The transition OL > CCL requires the following:

              - *parent*.coarse_track_thread exists and has a track
              - aformentioned track standard deviation is below CCL_transition_th (default 50 arcsec)
        ...
        - FCL: Fine closed-loop control. The measured position from the fine camera, returned by
          *parent*.fine_track_thread.track_x_y, is used (scaled to degrees) for the control signal.
          Gains FCL_P (proportional, default 1.0), FCL_I (integral, default 10 s), and rate limit FCL_speed_limit
          (default 180 arcsec/s) apply. The transition CCL > FCL requires the following:

              - *parent*.fine_track_thread exists and has a track
              - aformentioned track standard deviation is below FCL_transition_th (default 30 arcsec)

        - CTFSP: Coarse-to-fine spiral acquisition. Only available if CTFSP_enable is set to True. (defaults to False).
          It is recommended to use this only to automatically find the intercamera alignment, and then disable again for
          fastest possible acquisition. Enabling CTFSP has several effects:

              - When in CCL, the transition CCL > FCL will not occur as described above. Instead, when in CCL and the
                *parent*.coarse_track_thread.rms_error falls below CTFSP_transition_th (default 20 arcsec), the
                transition CCL > CTFSP  occurs.
              - When in CTFSP, CCL tracking parameters are used. If CTFSP_zero_integral = True is set (defaults to
                False) the integral term is set to zero.
              - When in CTFSP, the *parent*.coarse_track_thread.spot_tracker goal is supplemented by the goal offset
                with a spiral which has arms spaced by CTFSP_spacing (default 100 arcsec), at a rate of CTFSP_speed
                (default 50 arcsec/s), until a radius CTFSP_max_radius (default 500 arcsec) is reached, upon which the
                mode is changed back to CCL and the spiral offset is deleted.
              - When in CTFSP, the transition CTFSP > FCL happens *immediately* when *parent*.fine_track_thread exists
                and has a track.
              - When in FCL, if CTFSP_auto_update_CCL_goal = True, the *parent*.coarse_track_thread.spot_tracker goal
                is updated with the current track position, effectively calibrating the intercamera alignment. This only
                happens when *parent*.fine_track_thread.rms_error is below CTFSP_auto_update_CCL_goal_th (default 10
                arcsec). If CTFSP_disable_after_goal_update is True (the default), CTFSP mode will be disabled
                automatically after successful alignment.

    The availability of each mode can be controlled by setting CCL_enable, FCL_enable, CTFSP_enable to True or False,
    OL mode may not be disabled.

    Note:
        Integral windup protection is acheived by two means:

        1. The integral term is reset if the control signal desired is greater than the speed limit (disable by
           reset_integral_if_saturated=False).
        2. The integral term may grow (in magnitude) by a maximum value of integral_max_add (default 36 arcsec) per
           loop. (It may shrink by a maximum value of integral_max_subtract, default 360 arcsec).

    Tracking data is saved to a .csv file in the data_folder. The filenames are auto-generated with the start time (as
    an ISO timestamp) and _ControlLoopThread.csv.

    Args:
        parent (pypogs.System): The System to control.
        data_folder (pathlib.Path, optional): The folder for data saving. If None (the default) the folder
            *pypogs*/data will be used/created.
    ...
    def __init__(self, parent, data_folder=None, debug_folder=None):
        ...
        self._logger.debug('ControlLoopThread called with: parent=' + str(parent) + ' data_folder' + str(data_folder))
        # Data folder setup
        ...
        self._empty_state_cache = {'mode': None, 'freq': None, 'fb_alt': None, 'fb_azi': None, 'err_alt': None,\
                                   'err_azi': None, 'int_alt': None, 'int_azi': None, \
                                   'ct_has_track': None, 'ct_track_alt': None, 'ct_mean_alt': None,\
                                   'ct_track_azi': None, 'ct_mean_azi': None, 'ct_track_sd': None, 'ct_rmse': None,\
                                   'ct_search_x': None, 'ct_search_y': None, 'ct_search_rad': None,\
                                   'ft_has_track': None, 'ft_track_alt': None, 'ft_mean_alt': None,\
                                   'ft_track_azi': None, 'ft_mean_azi': None, 'ft_track_sd': None, 'ft_rmse': None,\
                                   'ft_search_x': None, 'ft_search_y': None, 'ft_search_rad': None}
        ...
        self._CTFSP_transition_th = 20.0 #Coarse RMS error needed to transition to spiral (arcsec)
        self._CTFSP_auto_update_CCL_goal = True #If True the CCL goal (intercam alignment) will be automatically
                                               #updated when fine tracking is achieved
        self._CTFSP_auto_update_CCL_goal_th = 10.0 #Fine RMS error needed to update the coarse goal (intercam alignment)
    ...
    def available_properties(self):
        ...
        return ('max_frequency', 'reset_integral_if_saturated', 'integral_max_add', 'integral_max_subtract',\
                'integral_min_rate', 'OL_P',\
                'OL_I', 'OL_speed_limit', 'CCL_enable', 'CCL_P', 'CCL_I', 'CCL_speed_limit', 'CCL_transition_th',\
                'FCL_enable', 'FCL_P', 'FCL_I', 'FCL_speed_limit', 'FCL_transition_th', 'CTFSP_enable',\
                'CTFSP_spacing', 'CTFSP_speed', 'CTFSP_max_radius', 'CTFSP_transition_th',\
                'CTFSP_auto_update_CCL_goal', 'CTFSP_auto_update_CCL_goal_th', 'CTFSP_disable_integral',\
    ...
    def reset_integral_if_saturated(self):
        """bool: Get or set if the integral term should be reset (zeroed) if control output is at the speed limit."""
    ...
    def integral_max_add(self):
        """float: Get or set the maximum error (arcseconds) allowed to be added (in magnitude) to the integral."""
    ...
    def integral_max_subtract(self):
        """float: Get or set the maximum error (arcseconds) allowed to be subtracted (in magnitude) to the integral."""
    ...
    def integral_min_rate(self):
        """float: Get or set the minimum target rate (arcsec per second) required to enable the integral term."""
    ...
    def CTFSP_enable(self):
        """bool: Get or set if Coarse-to-Fine Spiral auto-alignment is allowed. Disabled by default.
    ...
    def CTFSP_spacing(self):
        """float: Get or set the spacing between arms in Coarse-to-Fine Spiral auto-alignment in arcseconds."""
    ...
    def CTFSP_speed(self):
        """float: Get or set the speed in Coarse-to-Fine Spiral auto-alignment in arcsec per second."""
    ...
    def CTFSP_max_radius(self):
        """float: Get or set the maximum (reset) radius for Coarse-to-Fine Spiral auto-alignment in arcseconds."""
    ...
    def CTFSP_transition_th(self):
        """float: Get or set the CCL RMSE required to start Coarse-to-Fine Spiral auto-alignment in arcseconds."""
    ...
    def CTFSP_auto_update_CCL_goal_th(self, threshold):
        ...
        self._log_debug('CTFSP auto update threshold set to: ' + str(self.CTFSP_auto_update_CCL_goal_th))
    ...
    def CTFSP_disable_after_goal_update(self):
        """bool: Get or set if CTFSP should be automatically disabled when the alignment is finished."""
    ...
    def CTFSP_disable_after_goal_update(self, disable):
        ...
        self._log_debug('CTFSP disable after update set to: ' + str(self.CTFSP_disable_after_goal_update))
    ...
    def _run(self):
        ...
        if self._parent.target.start_time is None or apy_time.now() > self._parent.target.start_time:
        ...
        with open(data_file,'w') as file:
            ...
            writer.writerow(['T_UTC_ISO','T_ELAPSED','INDEX','MODE','TARG_MNT_ALT','TARG_MNT_AZ',\
                                'TARG_MNT_RATE_ALT','TARG_MNT_RATE_AZ','TARG_ENU_ALT','TARG_ENU_AZ',\
                                'MOUNT_MNT_ALT','MOUNT_MNT_AZ','COARSE_EXIST','COARSE_TRACK','COARSE_ALT_TRACK',\
                                'COARSE_ALT_MEAN','COARSE_AZ_TRACK','COARSE_AZ_MEAN','COARSE_SD',\
                                'COARSE_RMSE','FINE_EXIST','FINE_TRACK','FINE_ALT_TRACK','FINE_ALT_MEAN',\
                                'FINE_AZ_TRACK','FINE_AZ_MEAN','FINE_SD','FINE_RMSE','FB_ERR_ALT','FB_ERR_AZ',\
                                'FB_INT_ALT','FB_INT_AZ','FB_ANGVEL_ALT','FB_ANGVEL_AZ','FB_SATURATED',\
                                'FB_COMMAND_ALT','FB_COMMAND_AZ','FB_KP','FB_KI','FF_ALT','FF_AZ','REC_EXIST',\
                                'REC_POWER','REC_SMOOTH'])
        ...
        try:
            while not self._stop_loop and (loop_utctime < end_time if end_time is not None else True):
                # CONTROL LOOP #
                # Time info:
                loop_timestamp = seconds_since_start()
                loop_utctime = apy_time.now() #Astropy timestamp in UTC
                dt = loop_timestamp - last_timestamp if not last_timestamp is None else 0.0
                ...
                step = .2
                target_itrf_xyz = self._parent.get_itrf_direction_of_target(loop_utctime + [0, step]*apy_unit.s)
                # Convert through all coordinate frames (sys can do this for us, but we want the intermediates)
                target_mnt_altaz = self._parent.alignment.get_mnt_altaz_from_itrf_xyz(target_itrf_xyz)
                target_mnt_rate = (((target_mnt_altaz[:,1] - target_mnt_altaz[:,0] + 180) %360) - 180) / step
                target_mnt_altaz = target_mnt_altaz[:,0]
                # This one we keep just for fun ;)
                target_enu_altaz = self._parent.alignment.get_enu_altaz_from_itrf_xyz(target_itrf_xyz[:,0])
                # MOUNT position in the MNT frame
                mount_com_altaz = np.array(self._parent.mount.get_alt_az())
                mount_mnt_altaz = self._parent.alignment.get_mnt_altaz_from_com_altaz(mount_com_altaz)
                        ...
                        if ct_has_track and ct_track_sd is not None:
                            ...
                            if ct_track_sd < self._CCL_transition_th:
                                ...
                                if self._OL_Ki > 0 and self._CCL_Ki > 0:
                                    err_integral *= self._OL_Kp * self._OL_Ki / self._CCL_Kp / self._CCL_Ki
                elif mode == 'CCL':
                        ...
                        if ft_has_track and ft_track_sd is not None:
                            ...
                            if ft_track_sd < self._FCL_transition_th:
                                ...
                                if self._CCL_Ki > 0 and self._FCL_Ki > 0:
                                    err_integral *= self._CCL_Kp * self._CCL_Ki / self._FCL_Kp / self._FCL_Ki
                elif mode == 'CTFSP':
                    ...
                    if not self._FCL_enable or not ft_exists or not ft_has_track:
                        ...
                        if self._CCL_enable and ct_exists and ct_has_track:
                            ...
                            if self._FCL_Ki > 0 and self._CCL_Ki > 0:
                                err_integral *= self._FCL_Kp * self._FCL_Ki / self._CCL_Kp / self._CCL_Ki
                        else:
                            self._log_info('Switching FCL > OL.')
                            mode = 'OL'
                            if self._FCL_Ki > 0 and self._OL_Ki > 0:
                                err_integral *= self._FCL_Kp * self._FCL_Ki / self._OL_Kp / self._OL_Ki
                    elif self._CTFSP_enable and self._CTFSP_auto_update_CCL_goal:
                        # We are fine tracking and have spiral auto-align enabled. Check if we should update coarse
                        # tracker goal with the current (absolute) position.
                        ...
                        if ft_rmse is not None and ft_rmse < self._CTFSP_auto_update_CCL_goal_th:
                            ...
                            self._log_debug('CCL absolute mean: ' + str(ct_mean_abs_x_y) + ', old goal: ' \
                                            + str(self._parent.coarse_track_thread.goal_x_y))
                            if None in ct_mean_abs_x_y:
                                self._log_warning('Want to update coarse goal, but have no coarse mean.')
                            else:
                                try:
                                    ...
                                    self._parent.coarse_track_thread.spot_tracker \
                                                    .change_mean_relative(-ct_mean_abs_x_y[0], -ct_mean_abs_x_y[1])
                                except:
                                    self._log_warning('Failed to update goal', exc_info=True)
                                self._log_info('Auto updated coarse goal to: ' \
                                               + str(np.round(self._parent.coarse_track_thread.goal_x_y, 3)) + '.')
                ...
                if mode == 'OL':
                    ...
                    offset = self._OL_goal_x_y[::-1] + self._OL_goal_offset_x_y[::-1] #XY to altaz flip order
                    offset[1] /= np.cos(np.deg2rad(np.clip(mount_mnt_altaz[0],0,85)))
                    err_alt_az = (((target_mnt_altaz - mount_mnt_altaz - offset + 180) %360) - 180)
                ...
                if (not fb_ki) or (mode == 'CTFSP' and self._CTFSP_zero_integral) \
                    ...
                    add_integral = np.where(err_alt_az * err_integral > 0, #Pointing in the same direction \
                                            err_alt_az.clip(-self._int_max_add, self._int_max_add) * dt,\
                                            err_alt_az.clip(-self._int_max_sub, self._int_max_sub) * dt)
                ...
                (angvel_correction, saturated) = self._clip_feedback_rates(angvel_correction, speed_limit)
                self._log_debug('FB calculated: ' + str(angvel_correction) + ', saturated: ' + str(saturated))
                # Check if we are in spiral mode, then we should feed-forward the spiral rates
                if mode == 'CTFSP':
                    CTFSP_t += dt
                    # Spiral pos is in azi alt
                    new_spiral_pos = np.array(self._get_spiral_goal(CTFSP_t, spacing=self._CTFSP_spacing,\
                                                                    speed=self._CTFSP_speed, ramp=self._CTFSP_ramp,\
                                                                    delay=self._CTFSP_delay))
                    ...
                    angvel_correction[1] -= spiral_angvel[0] / np.cos(np.deg2rad(np.clip(mount_mnt_altaz[0],0,85)))
                ...
                angvel_total = self._get_safe_rates(angvel_correction + target_mnt_rate, mount_mnt_altaz)
                ...
                self._state_cache = {'mode': mode, 'freq': 1/dt if dt>0 else None, 'fb_alt': angvel_correction[0]*3600,\
                                     'fb_azi': angvel_correction[1]*3600, 'err_azi': err_alt_az[1]*3600,\
                                     'err_alt': err_alt_az[0]*3600,\
                                     'int_azi': err_integral[1]*3600, 'int_alt': err_integral[0]*3600,\
                                     'ct_has_track': ct_has_track, 'ct_track_alt': ct_track_alt_az[0],\
                                     'ct_mean_alt': ct_mean_alt_az[0],\
                                     'ct_track_azi': ct_track_alt_az[1], 'ct_mean_azi': ct_mean_alt_az[1],\
                                     'ct_track_sd': ct_track_sd, 'ct_rmse': ct_rmse, 'ct_search_rad': ct_search_rad,\
                                     'ct_search_x': ct_search_pos[0], 'ct_search_y': ct_search_pos[1],\
                                     'ft_has_track': ft_has_track, 'ft_track_alt': ft_track_alt_az[0],\
                                     'ft_mean_alt': ft_mean_alt_az[0],\
                                     'ft_track_azi': ft_track_alt_az[1], 'ft_mean_azi': ft_mean_alt_az[1],\
                                     'ft_track_sd': ft_track_sd, 'ft_rmse': ft_rmse, 'ft_search_rad': ft_search_rad,\
                                     'ft_search_x': ft_search_pos[0], 'ft_search_y': ft_search_pos[1]}
                # Save to file
                with open(data_file,'a') as file:
                    writer = csv_write(file)

                    writer.writerow([loop_utctime.isot, loop_timestamp, loop_index, mode, target_mnt_altaz[0],\
                                        target_mnt_altaz[1], target_mnt_rate[0], target_mnt_rate[1],\
                                        target_enu_altaz[0], target_enu_altaz[1], mount_mnt_altaz[0],\
                                        mount_mnt_altaz[1], ct_exists, ct_has_track, ct_track_alt_az[0], ct_mean_alt_az[0],\
                                        ct_track_alt_az[1], ct_mean_alt_az[1], ct_track_sd, ct_rmse,\
                                        ft_exists, ft_has_track, ft_track_alt_az[0], ft_mean_alt_az[0],\
                                        ft_track_alt_az[1], ft_mean_alt_az[1], ft_track_sd, ft_rmse, err_alt_az[0],\
                                        err_alt_az[1], err_integral[0], err_integral[1], angvel_correction[0],\
                                        angvel_correction[1], saturated, angvel_total[0], angvel_total[1], fb_kp,\
                                        fb_ki, ff_alt, ff_azi, rec_exists, rec_power, rec_power_smooth])
            ...
            if self._parent.coarse_track_thread is not None: self._parent.coarse_track_thread.stop()
            if self._parent.fine_track_thread is not None: self._parent.fine_track_thread.stop()
            if self._parent.receiver is not None and self._parent.receiver.is_running: self._parent.receiver.stop()
    ...
    def _get_safe_rates(self, desired_rates, curr_alt_az=None):
        """PRIVATE: Limit the desired rates to safe values such that the maximum speed and position of the Mount
        is not violated.

        Args:
            desired_rates (numpy.ndarray, tuple, list): length 2 array with desired altitude and azimuth rates in
                degrees per second.
            curr_alt_az (numpy.ndarray, tuple, list, optional): length 2 array with current telescope altitude and
                azimuth angles in degrees. If None (the default) *no position checking will be done*.

        Returns:
            numpy.ndarray: length 2 array with desired rates clipped to respect maximum rate and position.
        """
        ...
        if curr_alt_az is not None: #Check alt-azi limit cases
            ...
            if alt_lim[0] is not None and curr_alt_az[0] <= alt_lim[0]: rlim_alt[0] = 0
            if alt_lim[1] is not None and curr_alt_az[0] >= alt_lim[1]: rlim_alt[1] = 0
            if azi_lim[0] is not None and curr_alt_az[1] <= azi_lim[0]: rlim_azi[0] = 0
            if azi_lim[1] is not None and curr_alt_az[1] >= azi_lim[1]: rlim_azi[1] = 0
        return np.array([np.clip(desired_rates[0],*rlim_alt),np.clip(desired_rates[1],*rlim_azi)])
    ...
    def _get_spiral_goal(t, spacing=100, speed=100, ramp=5, delay=0):
        ...
        Args:
            ...
            speed (float, optional): speed in arcseconds per second to traverse spiral. Default 100.
            ramp (float, optional): Time constant in seconds for smooth start (by (1 - e**-t/ramp)).
        """
        f = lambda t: (1 - np.exp(-t / ramp)) * t
        t = np.where(t < delay, 0, t - delay)
        x = np.sqrt(spacing/np.pi*speed*f(t)) * np.cos(2*np.sqrt(np.pi/spacing*speed*f(t)))
        y = np.sqrt(spacing/np.pi*speed*f(t)) * np.sin(2*np.sqrt(np.pi/spacing*speed*f(t)))
        return np.vstack((x ,y)).squeeze()

    @staticmethod
    def _clip_feedback_rates(desired_rate, speed_limit):
        """Limit the desired feedback rate to the saturation limit. Returns limited rates and saturation flag.

        Args:
            desired_rate (numpy.ndarray, tuple, list): Desired rate.
            speed_limit (float): Rate limit in same units as desired_rate. If None no speed limit is applied.
...
class TrackingThread:
    ...
    SpotTracker will be created and attached if not given as input. The camera acquisition properties are set directly
    in the Camera and the spot detection parameters are set directly in the SpotTracker. They may be accessed (and set)
    by TrackingThread.camera and TrackingThread.spot_tracker respectively. If an image_folder is given to the
    TrackingThread the images read are saved (as .tiff) to the path, the maximum saving frequency can be limited by
    setting img_save_frequency (default 1 Hz) to avoid saving massive amounts of data.

    The TrackingThread will update on every image event received from the Camera, so the frequency is controlled by the
    Camera.frame_rate. If the thread is already busy, the image will be ignored and a frame drop warning logged.

    After setting up and starting the thread, read TrackingThread.track_alt_az to get the last measured position (error
    from the goal) of the satellite being tracked. See note below for coordinate definition. The tracker keeps an
    estimate of the mean and standard deviation of position, signal sum, and signal area which may be used to keep the
    track. Please read the SpotTracker (in this module) documentation carefully to set up the spot detection and
    tracking parameters.

    The TrackingThread can also pass feed-forward information to the SpotTracker to compensate for the movement
    of the telescope on the position of the target. The SpotTracker mean position will be moved at a speed of
    TrackingThread.feedforward_rate (tuple of floats; arcsec per second) but only if the step change is greater than
    TrackingThread.feedforward_threshold (default 10 arcsec). It is also possible to move the SpotTracker goal offset
    by a speed defined by TrackingThread.goal_offset_rate (arcsec per second) to follow a moving goal offset (i.e.
    during spiralling).

    Note:
        The TrackingThread has two distinct coordinate systems. The alt_az system measures positions relative to the
        defined goal and is (1) derotated with the Camera.rotation parameter and (2) switched (i.e. x and y becomes az
        and alt respectively) in order to coincide with the control loop's alt_az system compared to the SpotTracker
        x and y positions. However, many parameters (e.g. the goal and anything appended by _absolute) are in the
        same x_y coordinates of the SpotTracker *without any derotation*. All distance measurements are scaled by the
        Camera.plate_scale such that the outputs are directly in arcseconds.

    Args:
        camera (pypogs.Camera, optional): The Camera object to read from.
        spot_tracker (pypogs.SpotTracker, optional): The SpotTracker used to detect and track from the images. If this
            is not supplied an empty SpotTracker will be created.
        name (str, optional): A name for the TrackingThread.
        image_folder (pathlib.Path, optional): The folder for data saving. If None (the default) images will not be
            saved.
    ...
    def __init__(self, camera=None, spot_tracker=None, name=None, image_folder=None,
        ...
        # Start of constructor
        self._logger.debug('TrackingThread called with camera=' + str(camera) + ' spot_tracker=' + str(spot_tracker) \
                           + ' name=' + str(name) + ' image_folder=' \
    ...
    def _run(self):
        ...
        # Create logfile
        data_filename = Path(start_datetime.strftime('%Y-%m-%dT%H%M%S') + '_'  + self.name + '_TrackingThread.csv')
        data_file = self.data_folder / data_filename
        if data_file.exists():
            self._log_debug('File name clash. Iterating...')
            append = 1
        ...
        with open(data_file,'w') as file:
            writer = csv_write(file)
            writer.writerow(['T_UTC_ISO','T_ELAPSED','INDEX','IMG_SAVED','IMG_USED','HAS_TRACK','RMSE','X_TRACK',\
                                'X_MEAN','Y_TRACK','Y_MEAN','TRACK_SD','SUM_TRACK','SUM_MEAN','SUM_SD',\
                                'AREA_TRACK','AREA_MEAN','AREA_SD','FF_RATE_X','FF_RATE_Y','FF_STEP_X','FF_STEP_Y',\
                                'OFFS_RATE_X','OFFS_RATE_Y','OFFS_STEP_X','OFFS_STEP_Y'])
        start_timestamp = precision_timestamp()
        ...
        try:
            while not self._stop_running:
                ...
                if self._image_folder is not None:
                    ...
                    self._log_debug('Have image save path, checking rate limit, last timestamp:'+str(last_img_timestamp))
                    if self._img_save_frequency is None or last_img_timestamp is None \
                        ...
                        imgname = Path(start_datetime.strftime('%Y-%m-%dT%H%M%S') + '_' +self.name+'_img_'\
                                       +str(loop_index)+'.tiff')
                ...
                with open(data_file,'a') as file:
                    writer = csv_write(file)
                    writer.writerow([loop_datetime_utc.isoformat(), loop_timestamp, loop_index, int(img_saved),\
                                        int(img_used), int(has_track), rmse, x, mx, y, my, sd, s, ms, ss, a,\
                                        ma, sa, ff_rate[0], ff_rate[1], ff_step[0], ff_step[1], offs_rate[0],\
                                        offs_rate[1], offs_step[0], offs_step[1]])
                loop_index += 1
    ...
    def start(self):
        ...
        self._log_debug('Got start command')
        assert not None in (self.camera, self.spot_tracker), 'Must define a Camera and SpotTracker before starting'
    ...
    def img_save_frequency(self):
        """float: Get or set the maximum frequency to save images. If None (the default) all images will be saved."""
    ...
    def img_save_frequency(self, freq_hz):
        ...
        if freq_hz is None:
            ...
            assert isinstance(freq_hz, (int, float)), 'Save frequency must be None or a number (int or float)'
    ...
    def feedforward_rate(self):
        """"tuple of float: Get or set the speed (arcsec/sec) with which the SpotTracker mean position should be moved.
        """
    ...
    def goal_offset_rate(self):
        """"tuple of float: Get or set the speed (arcsec/sec) with which the SpotTracker goal offset position should be
        moved.
        """
    ...
    def goal_offset_rate(self, offset_rate):
        ...
        if offset_rate is None:
            ...
            try:
                ...
                self._log_warning('Failed to set goal offset rate from: ' + str(offset_rate), exc_info=True)
    ...
    def track_alt_az(self):
        """tuple of float: Get the latest alt and az position relative to the goal (derotated). None if no track or
    ...
    def track_x_y_absolute(self):
        """tuple of float: Get the latest absolute x and y position (relative to image centre). None if no track or
        latest update failed.
        """
        if not self.is_running: return (None, None)
    ...
    def mean_alt_az(self):
        """tuple of float: Get the mean (smoothed) alt and az position relative to the goal (derotated).
        None if no track.
    ...
    def mean_x_y_absolute(self):
        """tuple of float: Get the absolute mean x and y position (relative to image centre). None if no track.
    ...
    def pos_search_x_y(self):
        """tuple of float: Get or set the current search position. Will also set search radius to max_search_radius."""
        if not self.is_running: return (None, None)
    ...
    @property
    def goal_x_y(self):
        """tuple of float: Get or set the goal (in absolute x and y relative to image centre) the track is measured
        against.
...
class SpotTracker:
    ...
    The tracker works by keeping an estimate of the mean and standard deviation (SD) of position, signal sum, and
    signal area and searching for spots which fall within *sigma* SDs away from the mean. By default the position
    and signal sum are used with 5-sigma limits to keep the track. To change the limits see e.g
    SpotTracker.sigma_position; setting any of these to None will disable using that parameter to discern tracks. If
    more than one spot falls within the track limits the spot closest in position to the mean position will be used.
    *It is mandatory to set a reasonable maximum search radius (default 1000 arcsec).* This will also be used to
    initialise the position SD estimate. *It is recommended to set a reasonable minimum search radius (default None)*
    to at least one pixel's width. It is also possible to set maximum and minimum limits on the signal sum and area SD
    estimates, otherwise it will be limitied to between zero and the mean value of the respective signal.

    You may define a goal position against which the output should be measured. By default this is (0,0) which is the
    centre of the image. The units for all positions will be in the units you provide via plate_scale (default 1
    arcsec/pixel) to SpotTracker.update_from_image(). The goal and properties ending in _absolute are allways measured
    relative to image centre.

    The estimators use exponentially weighted moving mean and variance. SpotTracker.smoothing_parameter (default 10)
    defines how samples are weighted and roughly corresponds to the number of samples to average.

    If SpotTracker.success_to_start_track (default 3) spots are found in a row a new track will be established. If
    SpotTracker.fails_to_drop_track (default 10) failures occur in a row the track is dropped. If an update fails during
    tracking the tracking parameters will be penalised by SpotTracker.failure_sd_penalty (default 25%) to account for
    drift in the mean.

    A performance metric, the root-mean-squared error (RMSE), is provided which measures the position error relative to
    the goal (insted of the mean, as the SD does). By default this uses SpotTracker.smoothing_parameter with
    exponential averaging, but SpotTracker.rmse_smoothing_parameter may be defined to control this individually.

    You may elect to use your own spot detection system and directly update the tracker with position with
    SpotTracker.update_from_observation(). If this is done, the tracker will accept the updated position without
    checking if it falls within the defined tracking range.

    Note:
        The SpotTracker uses position information in (x,y) coordinates where (0,0) is the centre of the image, x
        increases to the right and y increases upwards. When images are used to update the tracker (via
        SpotTracker.update_from_image()) a plate scale should be supplied such that the output is in useful units, i.e.
        arcseconds.

    Args:
        name (str, optional): A name for the SpotTracker.
        data_folder (pathlib.Path, optional): The folder for data saving. If None (the default) the folder
            *pypogs*/data will be used/created. *TODO! Currently does nothing*
        debug_folder (pathlib.Path, optional): The folder for debug logging. If None (the default) the folder
            *pypogs*/debug will be used/created.
    ...
    def __init__(self, name=None, data_folder=None, debug_folder=None):
        ...
        # Start of constructor
        self._logger.debug('SpotTracker called with: name=' + str(name) + ' data_folder=' + str(data_folder))
    ...
    def available_properties(self):
        """tuple of str: Get the available tracking parameters (e.g. gains)."""
        return ('successes_to_track', 'fails_to_drop', 'failure_sd_penalty', 'max_search_radius', 'min_search_radius',\
                'position_sigma', 'sum_sigma', 'sum_max_sd', 'sum_min_sd', 'area_sigma', 'area_max_sd', 'area_min_sd',\
                'crop', 'downsample', 'filtsize', 'sigma_mode', 'bg_subtract_mode', 'image_sigma_th', 'image_th',\
                'binary_open', 'centroid_window', 'spot_min_sum', 'spot_max_sum', 'spot_min_area', 'spot_max_area',\
                'spot_max_axis_ratio', 'active_crop_enable', 'active_crop_padding')
    ...
    @property
    def crop(self):
        """tuple of int or or None: Get or set the cropping of incoming images. If None no cropping is applied.
    ...
    def active_crop_enable(self):
        ...
        If enabled and the SpotTracker has a track, the incoming image will be automatically cropped to only contain
        the search region (to speed up computation). This may have a slight effect on background subtraction and SD
        estimators.
        """
    ...
    def active_crop_padding(self):
        """int: Get or set the number of pixels to pad on each side of the search region for active cropping."""
    ...
    @property
    def sigma_mode(self):
        """str: Get or set the mode used to calculate the image sigma (standard deviation) after background
        subtraction.

        This value multiplied with SpotTracker.image_sigma_th will be used to threshold the image. If
        SpotTracker.image_th is not None (default is None), that will be used as the threshold instead, and sigma_mode
        and image_sigma_th have no effect.
    ...
    def sigma_mode(self, mode):
        ...
        mode = str(mode)
        allowed = ('global_median_abs', 'local_median_abs', 'global_root_square', 'local_root_square')
    ...
    @property
    def filtsize(self):
        """int: Get or set the filter size (width and height) used for local background subtraction and SD estimate."""
    ...
    def image_sigma_th(self):
        """float: Get or set the number of standard deviations of image noise used for image thresholding.
    ...
    def image_th(self):
        ...
        If this value is set (not None), the image standard deviation estimator and image_sigma_th will be ignored and
        the provided value used directly.
        """
    ...
    def binary_open(self):
        """bool: Get or set if binary opening (with 1-connected structure element) should be used to clean the
        thresholded image.
        """
    ...
    def centroid_window(self):
        """int or None: Get or set the second round centroiding window size. If None, the centroid will be calculated
        from the spot region in the thresholded image. If set, a square window of the given size will be used to
    ...
    def spot_min_area(self):
        """int or None: Get or set the minimum number of pixels in the thresholded image which constitute a star."""
    ...
    def spot_max_area(self):
        """int or None: Get or set the maximum number of pixels in the thresholded image which constitute a star."""
    ...
    def spot_max_axis_ratio(self):
        """float or None: Get or set the maximum ratio of major to minor axes for a region to constitute a star.
    ...
    def fails_to_drop(self):
        """int: Get or set the number of successive failed extractions after which a track is dropped."""
    ...
    def failure_sd_penalty(self):
        """float or None: Get or set the penalty in percent to give to the standard deviation on a failed extraction."""
    ...
    def max_search_radius(self):
        """float: Get or set the maximum search radius. Also sets initial standard deviation. Must be set; default 1000.
        """
    ...
    def min_search_radius(self):
        """float or None: Get or set the minimum search radius. Also sets initial standard deviation."""
    ...
    def position_sigma(self):
        """float or None: Get or set the number of standard deviations the search radius should be. Set None to disable
        tracking in position."""
    ...
    @property
    def sum_sigma(self):
        """float or None: Get or set the sigmal sum standard deviation threshold. Set None to disable tracking in sum.
        """
    ...
    @property
    def sum_max_sd(self):
        """float or None: Get or set the maximum (and initialisation) for signal sum standard deviation estimator."""
    ...
    def sum_max_sd(self, sum_max_sd):
        ...
        if sum_max_sd is None:
            ...
            assert isinstance(sum_max_sd, (int,float)), 'Input must be None or a scalar (int or float)'
    ...
    def sum_min_sd(self, sum_min_sd):
        ...
        if sum_min_sd is None:
            ...
            assert isinstance(sum_min_sd, (int,float)), 'Input must be None or a scalar (int or float)'
            self._sum_min_sd = float(sum_min_sd)
        self._log_debug('Set sum min SD to: '+str(self.sum_min_sd))

    @property
    def area_sigma(self):
        """float or None: Get or set the signal area standard deviation threshold. Set None to disable tracking in spot
        area.
        """
    ...
    def area_sigma(self, sig_area):
        ...
        if sig_area is None:
            ...
            assert isinstance(sig_area, (int,float)), 'Input must be None or a scalar (int or float)'
            self._area_sig = float(sig_area)
        self._log_debug('Set sigma area to: '+str(self._area_sig))

    @property
    def area_max_sd(self):
        """float: Get or set the maximum (and initialisation) for signal area standard deviation estimator."""
    ...
    def area_max_sd(self, area_max_sd):
        ...
        if area_max_sd is None:
            ...
            assert isinstance(area_max_sd, (int,float)), 'Input must be None or a scalar (int or float)'
    ...
    def area_min_sd(self, area_min_sd):
        ...
        if area_min_sd is None:
            ...
            assert isinstance(area_min_sd, (int,float)), 'Input must be None or a scalar (int or float)'
    ...
    @property
    def track_x_y(self):
        """tuple of float: Get the latest x and y position relative to the goal. None if no track or latest update failed."""
    ...
    @property
    def track_x_y_absolute(self):
        """tuple of float: Get the latest absolute x and y position (relative to image centre). None if no track or latest update failed."""
    ...
    @property
    def mean_x_y(self):
        """tuple of float: Get the mean (smoothed) x and y position relative to the goal. None if no track.
    ...
    @property
    def pos_search_x_y(self):
        """tuple of float or None: Get or set the x y position (absolute) where we search for track.
    ...
    def smoothing_parameter(self):
        ...
        - The total RMS Error smoothing can be set independently by SpotTracker.rmse_smoothing_parameter. If not, this
          value is used there as well.
        ...
        The SD estimator is initialised to given maximum values max_search_radius, sum_max_sd, area_max_sd if available,
        otherwise it is set to the magnitude of the first given value.
        """
    ...
    def rmse_smoothing_parameter(self):
        """float: Get or set the smoothing parameter. It roughly corresponds to the number of samples to average.
        ...
        - Exponential smoothing is used. rmse_smoothing_parameter is the *inverse* of 'alpha'. Each smoothed value s is
          defined from the measurements x by:
    ...
    def update_from_image(self, img, plate_scale=1):
        ...
        Will use tetra3.extract_star_positions() to find spots in the image with the extraction parameters which have been
        set in the SpotTracker. A spot falling within the current tracking range will be searched for and, if found, the
        tracker will be updated. If not found, the estimator SD will be penalised by SpotTracker.failure_penalty
        (default 10%).

        Args:
            img (numpy.ndarray): The image to update with.
            plate_scale (float, optional): Image plate scale (typically arcseconds per pixel). Defaults to 1
            (i.e. will output directly in pixels).

        Returns:
            bool: True if has_track *and* the provided image was successfully used to update the tracker.
        """
        imshape = img.shape
        return_value = False
        ds = 1 if self.downsample is None else self.downsample
        if plate_scale is None: plate_scale = 1
        assert isinstance(plate_scale, (int, float)), 'Plate scale must be a number (int or float) or None'

        if self.crop is not None:
            assert len(self.crop) in (2,4), 'Crop must be length 2 or 4 if tuple/list'
            if len(self.crop)==2:
                cr = list(self.crop[::-1]) + [-(self.goal_x_y[1] + self.goal_offset_x_y[1]) / plate_scale,\
                                         (self.goal_x_y[0] + self.goal_offset_x_y[0]) / plate_scale]
            else:
                cr = list(self.crop[1::-1]) + list(self.crop[3:1:-1])
        else:
            cr = None
        # If we have a track we only need to look in the allowed region anyway (if active crop is enabled)
        ...
        if self._has_track and self.active_crop_enable and search_rad is not None:
            ...
            if limit is not None and cr[0] > limit[0]: cr[0] = limit[0]
            if limit is not None and cr[1] > limit[1]: cr[1] = limit[1]
        ...
        if self._has_track:
            if self._sum_sig is not None:
                ...
                if self.spot_min_sum is not None: sum_min = max(self.spot_min_sum, sum_min)
                if self.spot_max_sum is not None: sum_max = min(self.spot_max_sum, sum_max)
            else:
            ...
            if self._area_sig is not None:
                ...
                if self.spot_min_area is not None: area_min = max(self.spot_min_area, area_min)
                if self.spot_max_area is not None: area_max = min(self.spot_max_area, area_max)
            else:
                area_min = self.spot_min_area
                area_max = self.spot_max_area

            ret = get_centroids_from_image(img,image_th=self.image_th,binary_open=self.binary_open,\
                                                filtsize=self.filtsize,crop=cr, downsample=ds,\
                                                min_area=area_min,\
                                                max_area=area_max,\
                                                min_sum=sum_min,\
                                                max_sum=sum_max,\
                                                ...
                                                return_moments=True,\
                                                sigma=self.image_sigma_th,\
                                                centroid_window=self.centroid_window)
            success = False
            if ret[0][:,0].size > 0:
                x = (ret[0][:,1]-imshape[1]/2) *plate_scale
                y = - (ret[0][:,0]-imshape[0]/2) *plate_scale #Image Y coordinates increase down!
                if not None in (x_search, y_search):
            ...
            ret = get_centroids_from_image(img,image_th=self.image_th,binary_open=self.binary_open,\
                                                filtsize=self.filtsize,crop=cr, downsample=ds,\
                                                min_area=self.spot_min_area,\
                                                max_area=self.spot_max_area,\
                                                min_sum=self.spot_min_sum,\
            ...
            if ret[0][:,0].size > 0:
                ...
                y = - (ret[0][:,0]-imshape[0]/2) *plate_scale #Image Y coordinates increase down!
                (x_search, y_search) = self.pos_search_x_y
                if self._auto_aquire: #Get 'self._succ_to_start' in a row to acquire
                    summ = float( ret[1][0] )
                    area = float( ret[2][0] )
                    x = float( x[0] )
                    y = float( y[0] )
                ...
                elif not None in (x_search, y_search):
                    dx = np.abs(x - x_search)
                    dy = np.abs(y - y_search)
                    index = (dx**2+dy**2).argmin()
                    x = float( x[index] )
    ...
    def update_from_observation(self, x, y, summ, area):
        ...
        if None in (x, y, summ, area):
            ...
            if None in (self._x_mean, self._y_mean, self._pos_variance, self._sum_mean, \
                ...
                self._pos_variance = (1 - self._alpha) * (self._pos_variance + self._alpha*(dx**2 + dy**2))
                ...
                if self._has_track:
                    ...
                    if self._max_search_rad is not None and self._pos_search_rad > self._max_search_rad:
                        self._pos_search_rad = self._max_search_rad
                    if self._min_search_rad is not None and self._pos_search_rad < self._min_search_rad:
    ...
    def penalize_track(self, percentage=25):
        ...
        if percentage is not None:
            ...
            if self._pos_search_rad is not None:
                ...
                if self._max_search_rad is not None and self._pos_search_rad > self._max_search_rad:
                    self._pos_search_rad = self._max_search_rad
            if not None in (self._sum_mean, self._sum_variance, self._area_mean, self._area_variance):
    ...
    def change_mean_relative(self, dx, dy):
        ...
        """
        if not None in (self._x_mean, self._y_mean):
            self._x_mean += dx
            self._y_mean += dy
        if not None in (self._pos_search_x, self._pos_search_y):
